...
def get_artifacts(base_url, username, password, output_file):
    ...
    endpoints = [
        "/api/search/aql",  # Standard endpoint
        "/artifactory/api/search/aql",  # Common alternative
        "/api/aql"  # Some older versions
    ]
    ...
    # If all endpoints failed, try the storage API as fallback
...
def try_fallback_method(base_url, username, password, output_file):
    ...
    try:
        ...
        print("Please check:")
        print("1. Your Artifactory URL is correct")
        print("2. You have proper permissions")
        print("3. The Artifactory version supports these APIs")
...
def export_to_csv(artifacts, output_file):
    ...
    print(f"Successfully exported {len(artifacts)} artifacts to {output_file}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--url', required=True, help='Artifactory base URL (e.g., http://artifactory.example.com)')
