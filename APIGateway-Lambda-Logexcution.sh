...
echo "Identifying All Webhook Stages in the Pipeline"
...
echo "Found $(echo "$WEBHOOK_STAGES" | jq -s 'length') Webhook Stages"

# Iterate through each webhook stage
echo "$WEBHOOK_STAGES" | jq -c '.' | while read -r stage; do
    ...
    API_LOG_GROUP=$(aws logs describe-log-groups --region "$AWS_REGION" --query "logGroups[?contains(logGroupName, '$API_GW_ID')].logGroupName" --output text)
    if [ -z "$API_LOG_GROUP" ]; then
        echo "No API Gateway log group found!"
        continue
    fi

    ERROR_LOGS=$(aws logs filter-log-events --log-group-name "$API_LOG_GROUP" --region "$AWS_REGION" --filter-pattern 'ERROR' --limit 1)
    ...
    LAMBDA_LOGS=$(aws logs tail "$LOG_GROUP" --region "$AWS_REGION" --since 1h --format short)
    ...
    # Save logs as an artifact for Spinnaker UI
